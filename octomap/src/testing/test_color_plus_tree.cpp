...
void print_query_info(point3d query, ColorPlusOcTreeNode* node) {
    if (node != NULL) {
        cout << "occupancy probability at " << query << ":\t " << node->getOccupancy() << endl;
        cout << "color of node is: " << node->getColor()
             << endl;
        cout << "label of node is: " << node->getLabel()
             << endl;
    }
    else
...
int main(int argc, char** argv) {
    ...
    for (int x=-30; x<30; x++) {
        for (int y=-30; y<30; y++) {
            for (int z=-30; z<30; z++) {
                ...
                ColorPlusOcTreeNode* n = tree.updateNode(endpoint, false);
                n->setColor(255,255,0); // set color to yellow
                n->setLabel(0,  255,0); // set label to something stupid like green
            }
        }
    }
    ...
    ColorPlusOcTree* read_color_plus_tree = dynamic_cast<ColorPlusOcTree*>(read_tree);
    EXPECT_TRUE(read_color_plus_tree);

    EXPECT_TRUE(tree == *read_color_plus_tree);
...

    {
        ...
        point3d query (0., 0., 0.);
        ColorPlusOcTreeNode* result = tree.search (query);
        ColorPlusOcTreeNode* result2 = read_color_plus_tree->search (query);
        std::cout << "READ: ";
        ...
        EXPECT_EQ(result->getLabel(), result2->getLabel());
        EXPECT_EQ(result->getLogOdds(), result2->getLogOdds());

        query = point3d(-1.,-1.,-1.);
        result = tree.search (query);
        result2 = read_color_plus_tree->search (query);
        print_query_info(query, result);
        std::cout << "READ: ";
        print_query_info(query, result);
        std::cout << "WRITE: ";
    ...
    // delete / create some nodes
    {
        ...
        ColorPlusOcTreeNode* newNode = tree.updateNode(newCoord, true);
        newNode->setColor(255,0,0);
        newNode->setLabel(255,0,0);
        ...
        ColorPlusOcTreeNode* parentNode = tree.search(newCoord, tree.getTreeDepth() -1);
        ...
        EXPECT_TRUE(tree.write("simple_color_plus_tree_ed.ot"));
